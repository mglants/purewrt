...
func applySection(c *Config, x struct {
	...
	case "main":
		...
		c.Settings.NetCheckEnabled = b(x.opts, "net_check_enabled", c.Settings.NetCheckEnabled)
		c.Settings.NetCheckCron = one(x.opts, "net_check_cron", c.Settings.NetCheckCron)
		c.Settings.NetCheckBytes = i(x.opts, "net_check_bytes", c.Settings.NetCheckBytes)
		c.Settings.ProxyGuardEnabled = b(x.opts, "proxy_guard_enabled", c.Settings.ProxyGuardEnabled)
		c.Settings.ProxyGuardCron = one(x.opts, "proxy_guard_cron", c.Settings.ProxyGuardCron)
		c.Settings.ProxyGuardMinMembers = i(x.opts, "proxy_guard_min_members", c.Settings.ProxyGuardMinMembers)
		c.Settings.ProxyGuardMinDownKbps = i(x.opts, "proxy_guard_min_down_kbps", c.Settings.ProxyGuardMinDownKbps)
		c.Settings.ProxyGuardProbeBytes = i(x.opts, "proxy_guard_probe_bytes", c.Settings.ProxyGuardProbeBytes)
		c.Settings.ProxyGuardMaxJitterMS = i(x.opts, "proxy_guard_max_jitter_ms", c.Settings.ProxyGuardMaxJitterMS)
		c.Settings.ProxyGuardActiveProbeInterval = i(x.opts, "proxy_guard_active_probe_interval", c.Settings.ProxyGuardActiveProbeInterval)
		c.Settings.ProxyGuardFleetProbeInterval = i(x.opts, "proxy_guard_fleet_probe_interval", c.Settings.ProxyGuardFleetProbeInterval)
		c.Settings.ProxyGuardQuarantineSeconds = i(x.opts, "proxy_guard_quarantine_seconds", c.Settings.ProxyGuardQuarantineSeconds)
	...
	case "mesh":
		...
		c.Mesh.OverlayIPv4 = one(x.opts, "overlay_ipv4", d.OverlayIPv4)
